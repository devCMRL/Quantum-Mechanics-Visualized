"""Chapter 1 — standard postulates mapped to course chapters."""

import streamlit as st

st.set_page_config(page_title="Ch 1 — Postulates roadmap", layout="wide")

st.title("Chapter 1 — Five postulates (roadmap)")
...
**Chapters:** **2** (finite-dimensional $\mathbb{C}^N$), **6–8** and **14** (position representation on a line), **13** (qubit geometry).
...
**Chapters:** **4** (Pauli matrices on $\mathbb{C}^2$), **3** (a Hamiltonian as a $2\times2$ observable), **7–8** (Hamiltonian eigenfunctions in concrete potentials).
...
**Chapters:** **3** (Born weights and repeated sampling of one observable).

### 4. Time evolution (Schrödinger equation)
Isolated systems evolve unitarily: $|\psi(t)\rangle = e^{-iHt/\hbar}|\psi(0)\rangle$.

**Chapters:** **14** (superposition of box eigenstates with time-dependent relative phases). Chapter **5** addresses static uncertainty relations at a fixed time, not unitary evolution.
...
Chapters **2–5** treat finite-dimensional algebra, measurement, and Pauli-based uncertainty; chapters **6–14** treat one-dimensional stationary and time-dependent problems plus one spherical-harmonic visualization.
"""
)

st.markdown(
    """
**Suggested order:** 2 → 3 → 4 → 5, then 7 and 14 for eigenfunction expansions and their time dependence.
"""
)
